...
<body>
   ...
   -->
		<c:if test="${quiz.id != null}">
			<form method="POST"
				action="<%=request.getContextPath()%>/UpdateQuiz.do?quizId=${quiz.id}">
		</c:if>
		<c:if test="${quiz.id == null}">
			<form method="POST"
				action="<%=request.getContextPath()%>/AjouterQuiz.do">
		</c:if>
		<div class="form-group">
			<c:if test="${listeQuestions == null}">
				<div class="jumbotron">
					<h1>Ooops!!</h1>
					<p>
						Pas de question dans la base de données.<br>Créez la première
						question.
					</p>
					<a href="<%=request.getContextPath()%>/VueQuestionAdmin.do"
						class="btn btn-primary"> <span
						class="glyphicon glyphicon-plus"></span> Ajouter une Question
					</a>
				</div>
			</c:if>
			<c:if test="${listeQuestions != null}">
				<div class="row">
					...
					<div class="col-xs-12 col-sm-4">
						...
						<c:if test="${quiz.id != null}">
							<input type="text" class="form-control" name="libelleQuiz"
								id="libelleQuiz" value="${quiz.libelle}">
						</c:if>
					</div>
					<div class="col-md-4">
						<a href="<%=request.getContextPath()%>/VueQuestionAdmin.do"
					...
					<div class="col-xs-12">
						<p class="text-danger h4">
							<html:errors property="err.inputs" />
						</p>
						<hr>
					</div>
					<div class="col-xs-12">
						<table class="table table-bordered table-hover">
                          ...
                          <c:if test="${question.id == existing_q.id}">checked</c:if>
										...
										<td><a
											href="<%=request.getContextPath()%>/SupprimerQuestion.do?idQuestion=${question.id}">
												<span class="glyphicon glyphicon-remove"></span>
										</a></td>

										<td><a
											href="<%=request.getContextPath()%>/EditionQuestion.do?idQuestion=${question.id}">
					...
					<div class="col-xs-6 col-md-3 col-lg-2">
						<a href="<%=request.getContextPath()%>/VueQuizAdmin.do"
							class="btn btn-default btn-block">Annuler</a>
					</div>
				</div>
			</c:if>
		</div>
