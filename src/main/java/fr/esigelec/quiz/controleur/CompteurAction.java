...
/**
 * action qui correspond au passage à la question suivante de la part de
 * l'animateur si on est dejà à la dernier question on revient sur cette
 * derniere
 * 
...
public class CompteurAction extends Action {
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		if (personne.getDroits() == Personne.ADMIN) {
			...
			// noQuestionCourante correspond au no de la question courante (qui
			// commence à zero).
			int noQuestionCourante = Integer.parseInt(request
					.getParameter("compteur"));

			QuizDAOImpl quizdaoimpl = new QuizDAOImpl();
			// recuperation du quiz en session
			Quiz quiz = (Quiz) session.getAttribute("quiz");
			Question question;
			// si on est pas à la derniere question on peut passer a la question
			// suivante.
			...
			// on remet la date de debut question à la date courante (vu que
			// c'est une nouvelle question affichée)
			...
			// on remet l'etape de la question à 1 (opur que les joueurs
			// puissent repondre)
			quiz.setEtape(1);
			...
			// on envoie la date de debut de la question à la vue (date du
			// serveur bien sur ;))
